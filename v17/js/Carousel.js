...
constructor(element, options = {}) {
  ...
  this.options = Object.assign(
    ...
    if(this.options.movingAlone === true){
  ...
  createNavigation() {
    ...
    //positionnement bouton prev
    this.prevButton.style.left = "0";
    this.prevButton.style.transform = "translate(-50%, -50%)"
    //positionnement bouton next 
  ...
  createPagination() {
    ...
    for (
      ...
      this.onMove((index) => {
        const activeButton =
          buttons[
            Math.floor((index - this.offset) / this.slidesToScroll)
          ];
        if (activeButton) {
  ...
  gotoItem(index, animation = true) {
    ...
    if(this.options.infinite){
      ...
      if (
        this.currentItem >=
          this.nbItems + this.offset - this.slidesVisible &&
        index === this.offset
      ) {
        index =
          this.currentItem + this.slidesVisible;
      } else if (
        this.currentItem === this.offset &&
        index === this.nbItems + this.offset - (this.slidesVisible - (this.nbItems % 2))
      ) {
        index =
          this.currentItem - this.slidesVisible;
      } 
    ...
    if (index < this.offset) {
      if (this.options.loop) {
        index =
          this.items.length - (this.slidesVisible - (this.nbItems % 2));
  ...
  moveAlone(){
    if(this.movingAlone === true){
      this.next();
      setTimeout(() => this.moveAlone(), 5000);
    }
  }
}
